...
class DuckingEngine {
    ...
    for (const target of targets) {
      ...
      if (this._instanceCounts[target]) {
        if (wasFirstInstance) {
          ...
          await this._enqueueVolumeOp(target, effectiveVolume, 'apply');
    ...
    for (const target of targets) {
      ...
      if (this._instanceCounts[target] && this._totalInstances(target) > 0) {
        ...
        if (remainingTotal === 0) {
          ...
          await this._enqueueVolumeOp(target, restoreVolume, 'restore');
          ...
          await this._enqueueVolumeOp(target, effectiveVolume, 're-evaluate');
  ...
  _enqueueVolumeOp(target, volume, context) {
    ...
    // Return the chained promise so handleEvent resolves only after this
    // op's write has landed (the public contract: callers can await
    // handleDuckingEvent when ordering matters). Errors are swallowed by
    // the catch above — this never rejects. Serialization (F-SHOW-06) is
    // unaffected: the next op still chains on _opQueues[target].
    return this._opQueues[target];
  }

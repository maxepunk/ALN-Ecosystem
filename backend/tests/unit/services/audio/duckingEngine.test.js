...
describe('F-SHOW-06: per-target op serialization', () => {
  it('serializes duck-apply and restore so apply always completes before restore', async () => {
    ...
    // Start ducking WITHOUT awaiting — handleEvent resolves only after its
    // write lands (awaited-completion contract), and the apply write is
    // deliberately barriered here to prove a fast restore cannot overtake it
    const startPromise = engine.handleEvent('sound', 'started');

    // Let the capture microtask run so the apply op is enqueued (and blocked)
    await new Promise(resolve => setImmediate(resolve));
    expect(port.setVolumeLive).toHaveBeenCalledTimes(1); // apply issued, blocked

    // Complete — the restore must chain AFTER the blocked apply
    const stopPromise = engine.handleEvent('sound', 'completed');

    // Release the apply — both ops should now run in order
    resolveFirst();

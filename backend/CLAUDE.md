...
**PID-file isolation (`ALN_PIDFILE_DIR`) — SAFETY CRITICAL:**

`ProcessMonitor` keeps only the BASENAME of a caller's `pidFile` and places it in `process.env.ALN_PIDFILE_DIR` (default `/tmp`); `src/utils/displayDriver.js` resolves its Chromium pidfile through the same helper at each use. Production leaves the variable unset and keeps `/tmp`. `jest.config.base.js` allocates one temp dir per jest run (unit, contract AND integration, with a per-worker fallback in `jest.setup.js`), removed in `jest.globalTeardown.js`. `tests/e2e/setup/test-server.js` allocates one per Playwright parallel slot (`TEST_PARALLEL_INDEX`, falling back to the harness pid) and passes it to every spawned orchestrator, exposing it as `pidFileDir` on the returned info object.

Before this (2026-09-15) every pidfile path was a fixed `/tmp/aln-pm-*.pid` shared by the PM2 production orchestrator, every E2E-spawned orchestrator and any jest suite that builds a real ProcessMonitor — so running the unit suite on the production box killed the live show's VLC, because `start()` → `_killOrphan()` read the production pidfile and SIGTERMed it. The E2E dir is scoped per parallel slot, NOT per spawn or per worker process (Playwright forks a NEW worker for each retry), because a SIGKILLed orchestrator orphans its VLC and only the next spawn's `_killOrphan()` reaps it — per-spawn dirs would silently break the one-VLC invariant in `flows/00-smoke-test`.
...
- **Any suite that calls `initializeServices()` MUST pair it with `afterAll(cleanupInitializedServices)`** (`tests/helpers/service-reset.js`). Init starts REAL ProcessMonitor children — `dbus-monitor --system` (bluetoothService) and `pactl subscribe` (audioRoutingService) — and nothing else stops them: `resetAllServices()` only covers session/transaction/videoQueue/offlineQueue. Without the pairing they survive jest as PPid-1 orphans and accumulate on the host, 2-3 per full run (fixed 2026-09-15).
- **Enforced automatically:** the jest test environment (`tests/helpers/jest-environment-guarded.js`, set as `testEnvironment` in `jest.config.base.js`, so unit, contract and integration alike) runs `assertNoLiveChildren()` (`tests/helpers/live-children-guard.js`) in its `teardown()`, which executes after every hook a test file declared, including a root-level `afterAll`. It scans `/proc` for real children of the jest worker (parent pid check; mocked spawns have no real process), SIGKILLs them and fails the file with "This test file leaked N live child process(es)" listing `pid cmdline`. Seeing that message means the file started a real process and never stopped it — add the missing cleanup; the process itself has already been killed. The killing function takes no arguments by design (an owner override once killed the operator's desktop session, 2026-09-18).
...
- `DISPLAY_DRIVER=off` makes the driver inert (no launch, no xdotool/wmctrl; `ensureBrowserRunning()`/`showScoreboard()` resolve false, `hideScoreboard()` true; one warn at first use). `jest.config.base.js` sets it for unit, contract and integration — 22 suites used to put a real kiosk on the show TV pointed at port 3000 per file (2026-09-18). E2E and production leave it unset.
...
- `_doLaunch()` two-stage orphan recovery before spawning: (1) SIGKILL via PID file (`aln-pm-scoreboard-chromium.pid` in `ALN_PIDFILE_DIR`, `/tmp` in production — resolved at use via `src/utils/pidFile.js`, see PID-file isolation under Testing; verified against `/proc/pid/cmdline`), (2) `pkill -9 -f chromium.*--kiosk` fallback to catch children reparented to init. 2s wait after either kill for lock release. 1s alive-check after spawn detects early crashes (e.g., single-instance lock conflict).
...
**Reusable Utilities:**
- `src/utils/processMonitor.js` — Self-healing spawned-process wrapper (spawn, line-buffer, exponential backoff restart, orphan prevention, PID-file orphan recovery after SIGKILL). Manages 5 processes: VLC player, 2 D-Bus monitors (VLC/BlueZ), pactl subscriber, MPD daemon. PID files are named `aln-pm-*.pid` and live in `ALN_PIDFILE_DIR` (default `/tmp`, which is what production uses). **`stop()` is intentionally sync** — making it async cascades through 12+ methods (all service cleanup/reset methods, systemReset.js, test helpers). Exit handler uses captured `proc` closure + SIGKILL (not `this._proc` + SIGTERM) so it works after `stop()` nulls `_proc`. Exit handler removed in the `close` handler (not `stop()`) so the safety net persists until child is confirmed dead.
...
- VLC is managed by ProcessMonitor in `vlcMprisService.init()` (not PM2). `reset()` preserves VLC ProcessMonitor (process kept running), `cleanup()` stops it. PID file at `/tmp/aln-pm-vlc.pid` in production (`ALN_PIDFILE_DIR` unset — see PID-file isolation under Testing).

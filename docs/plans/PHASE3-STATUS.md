...
| **A3 slice 3b (pack-driven formatting)** | 🔨 **OPEN — census VERIFIED, design DRAFT, decision-free core may build ahead** (branch `claude/phase3-a3-slice3b` chained from the 3a tip `4dee0aa`). Dedicated census sweep re-verified the 3a §3 handoff against the post-3a tree: every claimed site EXISTS (2 line drifts, both in 3a-touched files; `formatCurrency` call sites 17 not ~15; report formatters 3 not 2) + 11 additional findings — notably `scoring.display` has ZERO readers and BOTH normalizers actively DROP it; toy-heist already declares a genuinely divergent format (`#,### cr`, suffix unit); the E2E page objects hardcode the `[$,]` strip regex ×4 across 2 files (would NaN on non-`$` prefixes); `GameOpsRenderer` star sites have NO test pin and `:484` throws RangeError for rating>5; the rating scale is schema-FROZEN at 5 (`baseValues` required keys 1..5). Design `2026-08-21-phase3-a3-slice3b-formatting.md`: R-3b-1 (recommended) — `scoring.display.format` becomes the driving spec with a minimal grammar (one `#,###` signed grouped-integer token + literal affixes; schema pattern + gate refusal for undrivable formats), verbatim discipline preserves ALN byte-identity incl. the `$-25,000` negative quirk; report generator (B9 golden), validators family, debug logs, PWA (L3), and award-message number formatting stay OUT. **TWO OWNER QUESTIONS (HELD):** Q-3b-1 award-message number formatting (recommend additive `{pointsFormatted}` placeholder), Q-3b-2 star glyph declaration (recommend keep per-surface baked glyphs; 3c/theme territory). Estimate ~2-3 sessions. |

...
| **A3 slice 3a (pack-declared strings)** | 🔨 **OPEN — census COMPLETE, design DRAFT, decision-free core building ahead** (branch `claude/phase3-a3-slice3a` chained from the slice-2b tip `be24d96`, PR #23 draft CI vehicle). Workflow census (6 parallel surface agents + synthesis, sites verified against the working tree): **160 3a sites / 50 → 3b / 16 → 3c / 35 out-of-scope**; design doc `2026-08-21-phase3-a3-slice3a-strings.md`. Delivery mechanism settled by EXISTING headroom (not a decision): a `strings.json` sidecar — game.schema.json already defines the strings/theme filename pointers and build-pack-manifest already assigns their roles; the work is authoring + strings.schema.json + activation-gate validation + consumers (scanner packLoader, scoreboard pack-files fetch, config-tool /api/config identity exposure). Pre-fixes in scope: the "Case File" FUNCTIONAL xdotool selector (shared config consumed by displayDriver + scoreboard — NO test pins it today, a rebrand passes CI and silently breaks HDMI control), the scoreboard hardcoded admin password (+5 co-located test/dev literals), the F-SHOW-29 idle-loop literal. **FIVE OWNER QUESTIONS (census §5, HELD FOR OWNER):** Q1 entities.label ratification (pack declares "Account/Accounts", every surface says "Team" — blocks the ~20-site family), Q2 claimed-label mechanism (modes[].claimedLabel+icon vs strings.modes.<id>.*), Q3 token-noun boundary, Q4 report wording default-OUT (B9/slice-7 contract), Q5 scoreboard theming depth. Estimate ~5-7 sessions (census-based; +1 if Q4 opts the report in). R13 brake citation in the doc. |

...
### Venue step 2 (2026-09-15) — the E2E "30 s" is bench-only; the in-process path is 0.32 s

Before running, three hypotheses were tested against a faithful reproduction of the E2E condition WITHOUT the orchestrator (idle harness-style `cvlc --intf dummy` owning the bus name + a production-argument instance beside it, OpenUri from another client, every PropertiesChanged timestamped): OpenUri plays on the harness-style instance (assumption confirmed); 5 signals in 28.5 s, median gap 170 ms, no two within 100 ms → **debounce starvation refuted**; every signal carries a sender → the filter gap cannot hide drops (**refuted**); PlaybackStatus=Playing at +0.154 s → **late VLC emission refuted**. (Pipe buffering was refuted earlier: ~20 ms delivery.)

Then flow 30 ran at the venue with `TEST_LOG_LEVEL=debug` and four new debug-level trace lines (parser signal → merged apply → state writer → loaded-wait handler). **Result: 1 passed, first attempt, all seven phases**, and the cue step's in-process trace: play command 16:03:12.679 → Metadata+Playing signal parsed 12.943 → apply (100 ms merge) 12.943 → `state:changed` 12.947 → loaded-wait saw playing 12.952 → "started via VLC" 13.000 → first progress tick 14.025 → completed 16:03:43.0. **0.32 s command→observed; no gap anywhere in the process.**

**What this means, stated carefully.** The 29.4–29.7 s gaps were observed only on the bench (no HDMI attached, Xorg with no connected output). Every venue observation — production single instance (step 1), the bus-level two-instance probe, and the full E2E two-instance run — is fast. The variable that differs is the display, not the instance count. The mechanism inside VLC (most plausibly video-output creation on a display-less X server delaying the player's Playing report for the harness instance, which is launched with no `--vout` flag) is a HYPOTHESIS, not established: the bench probe that launched VLC with the file on its command line reported Playing in 20 ms, so the OpenUri-on-idle-instance path at the bench is the untested case. Bench test to settle it: repeat the two-instance OpenUri probe with no display; then again with `--vout=dummy` on the harness instance.

**Consequence for C2.** Identity-based addressing is still correct hygiene (three-worker runs, orphan hijack, honest bus-name ownership), but it is NO LONGER justified as the fix for the flow-30 flake; the flake's trigger is the bench's missing display, and the cheapest correct harness change may be `--vout=dummy` (or `VLC_HW_ACCEL`-style opt-in) for the harness instance when no display is connected. Decide after the bench probe.

**New show-relevant finding from the same trace:** the HEVC player-scanner clip (`kai001.mp4`) reported `length: 0` from play start until its END (Metadata with `mpris:length` arrived only at completion), so `monitorVlcPlayback` emitted NO `video:progress` for its whole playback (the `length > 0` guard). A player-scanner video needs no ticks, but a VIDEO-DRIVEN CUE does: a compound cue whose video reports no length until the end would sit in boundary mode with none of its timeline entries firing. See the July-18 ENDGAME check below.

**July-18 live ENDGAME check (from `logs/combined.log`, port-3000 show window):** cue fired 23:27:48.8 → `video:queue:add` 23:27:49.99 → "started via VLC" 23:27:50.28 (+0.3 s) → progress tick +1.0 s → timeline entries fired at +180 s (23:30:51), +300 s, +390/391 s, +540 s, +600 s → video completed 23:38:49 → cue completed +4 ms. **The production video-driven path worked end to end at the last real show.** Also visible there: `restore-after-video` fired three times right after the ENDGAME video (the condition bug fixed today), masked because the ENDGAME timeline's own last entry is `scene.game`.

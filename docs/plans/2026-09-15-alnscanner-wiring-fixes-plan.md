...
## W9 (deferred, traced 2026-09-15) — video-driven cue completion: harness, service, engine

Origin: flow 30 step 1.6.6 (`e2e-video-compound`) — see review doc → "Follow-ups found during E2E verification". Read-only consequence trace done; nothing implemented. Owner decides scope.

**Corrected model.** `capturedVideoTokenId` (timelineRuntime.js ~:234) is ALWAYS null in production because both video cues in `cues.json` queue by `videoFile`, so every F-SHOW-08 correlation guard is inert: any video's progress/completion already drives any boundary-mode cue. The two defects are one chain: a stale cached `stopped` in `vlcMprisService` survives `maxNonPlayingChecks=1`, so `monitorVlcPlayback` calls `completePlayback` ~2s in and emits `video:completed` with zero progress ticks; the boundary-mode cue then ignores it.

**Ranking (risk × value):**
1. **C — harness only (do first).** Every flow calls `setupVLC()` BEFORE `startOrchestrator()`, so the harness instance wins the well-known bus name and the orchestrator's own ProcessMonitor instance lands on `.instanceNNN`, its signals dropped by the sender filter. There is no adoption path in `vlcMprisService.init()` (it only logs "Existing VLC processes found"). Change: stop spawning the harness VLC; let ProcessMonitor own the only instance (`VLC_HW_ACCEL=""` headless); replace the `vlcInfo.type === 'real'` gate with `getCapabilities(...).vlc`; `stopOrchestrator()` already kills/respawns VLC per test via `vlcService.cleanup()`. Production untouched. Tests: rerun show-control + display-control flows; assert exactly one `cvlc` during a test and no "Existing VLC processes found" line.
2. **B — service fallback.** `getStatus()` has exactly five callers, all in the video-queue path (`vlcMprisService.js:169`; `videoQueueService.js:288, :309, :339, :557`); `getState()`/`service:state`/`sync:full` are cache-only, so the wire contract is untouched. Change: when cached `length === 0` or cached state disagrees with an outstanding play, do a direct `PlaybackStatus` + `Metadata` D-Bus read (bypasses the sender filter) and route the refresh through `_processStateChange` so `waitForVlcLoaded`'s `state:changed` resolves. Conditional only (self-extinguishes once metadata lands); always-on would triple D-Bus forks per second. Load-bearing risk: `_parseMetadata` expects `dbus-monitor` text; verify against real `dbus-send --print-reply` output on the Pi and add that as a fixture. Tests: unit on `getStatus` (length 0 → fallback recovers; length > 0 + playing → no extra call = production no-op; monitor emits `video:progress` after recovery).
3. **A — engine safety net (last).** `handleVideoLifecycle`: accept `completed` for a boundary-mode cue when the completed item is the one its own `video:queue:add` produced (correlate on filename → `queueItem.tokenId` = filename minus extension, `videoQueueService.js:728-770`), take the anchor from the event's `VideoQueueItem.getPlaybackDuration()` (NOT the frozen boundary `elapsed`, or the police cue's 180/300/390s entries fire that long after the video ends), fire remaining entries at their `at` under post-video clock anchoring (skipping leaves the cue resident forever per `checkCompletion`; marking-without-firing drops the 600s `scene.game` restore), and use a distinct `videoResolved` flag rather than `videoStarted` (cascade keys in stop/pause/resume). Note post-video depends on `gameclock:tick`. Previously-dead `cue:completed` chains would fire for the first time (none configured today). Remove the always-truthy arrow at ~:459. Tests: extend `timelineRuntime-e5.test.js` — completion-before-progress with no pending entries completes once; with pending entries anchors at playback duration and fires at correct offsets; unrelated video's completion does not complete; stop during post-video does not double-skip.

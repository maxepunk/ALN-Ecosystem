...
| R11 | The kit's network values were already in the repo (owner, checkpoint 1: "don't you have the ssid, ip, dns name in our repos already?"); T1b authors the ALN profile's `network` block: SSID `aboutlastnetwork`, IP `192.168.0.191` (owner-stated production values; the ESP32 sample config's `Sidewinder` / `10.0.0.177` are not the production values), `localDnsOverride: true` per CONTEXT.md §5 (superseded 2026-09-12: DNS is not set up at the venue; the profile carries `false`; DNS and the certificate are deferred to ROADMAP row 8.19, see R23). The DNS name is NOT in the repo as a decision (the July design's `play.aboutlastnightgame.com` sits beside placeholder SSID/IP values) — `orchestratorName` stays omitted until the owner states it. SEC-25's "owner task" narrows to that one value | owner + orchestrator, 2026-09-12 |
...
### 2026-09-12 — harness minimum CLOSED (P21 per-worker data and log directories, P22 the log guard) — merged; the lane worktrees stand

- Commits `1b2cccb`, `30d1514`, `f778f15`, `490c83f` on task branch `claude/nice-curie-hescfv-harness-minimum` (base `33b2128`), merged `--no-ff` into the designated branch at `209916b` and pushed; CI run 303 on the merge was in progress when this entry was written (its verdict goes in the next entry). Brief: `briefs/2026-09-12-harness-minimum.md`; report in the scratch directory (`harness-minimum-report.md`).
- Tests (the orchestrator's fresh gate on `490c83f`): unit + contract 3123/3123, ratchet 85/85, lint clean, integration 38 suites 349/349. The implementer's own runs: the RED evidence for both pins, Tier L toy leg 3 workers 140/0/0. The merge touched only the task's five backend files and the designated branch had gained no backend change since the base, so the gate covers the merged tree.
- Review (Opus): spec pass, quality approved; five minors. Hostile tester (R26 point 3; Opus, one refuter per finding): seven findings, all seven refuted on evidence, seventeen attacks held. Fix round 1 (`490c83f`: F1 the `setMaxListeners` sentinel, F2 the guard line at `error`, F3 a tail read on the RED path, F4 a stale comment, H1 the overbroad test header) verified by a scoped re-review: all addressed, no new breakage; the test-only export on the logger object accepted under the repo's `_resetForTesting` convention.
- Rulings 29–31, each with its cost if wrong:
  29. A merge into the designated branch may be performed inside `.worktrees/docs` while a reading agent writes a document there, when the merge touches no file that agent writes; the worktree is not removed for it. — cost: none; a conflicting write would have refused the merge.
  30. Lanes run in parallel only across disjoint file sets (ruling 6 generalised): the profile check builds first (owner ruling, §4) in the main checkout on `claude/nice-curie-hescfv-profile-check`; the self-heal lane may build beside it (disjoint files); the credentials lane starts after the profile check merges (both edit `asyncapi.yaml`); the supervisor lane after the dormancy-core survivors task (R26 point 1); the arms after the profile check. — cost: if wrong, a merge conflict caught at the merge gate.
  31. R26 point 2 applies to the profile-check brief although its review passed before R26 was adopted: a red team runs on it before its implementer. The self-heal brief's flow is renumbered `33-pack-self-heal` (the profile check owns `32`). — cost: about one review round of wall clock.
- Findings carried (rig hygiene, the P25/P27 family, ledger rows): an audio-init off switch for harness children (every orchestrator boot runs `pkill -f "pactl subscribe"` and card probing machine-wide; pre-existing); the harness-side logger in the Playwright process still writes `backend/logs` (H6); the slot root under `/tmp/aln-e2e-env` is predictable and `clearSessionData` follows a planted symlink (H7); `/tmp/aln-e2e-env/w*/` is never cleaned (P25); the MPD residue assertion is vacuous in this container; non-EPIPE stream errors are swallowed by design. Dead surface: `persistenceService.resetMemoryStorage()` has no caller after the memory branch removal (F5) → the sweep's cleanup.
- Environment for the record: the container restarted about 23:21Z (the second restart of the day); files survived, every rig daemon died; the arms were brought back with the recipe (`2026-09-12-container-baseline.md` §7), probe identical to the recorded baseline. Three lane worktrees cut from `209916b` with the scripted recipe: `.worktrees/credentials` (parent branch), `.worktrees/supervisor` and `.worktrees/self-heal` (parent branch plus an ALNScanner branch of the same name), submodules at the pins, `node_modules` symlinked, one jest sanity run each side green.
- Next: red teams on the credentials, self-heal and profile-check briefs (R26 point 2); their standing findings edited into the briefs; the profile-check build.

### 2026-09-13 — research CLOSED (revision 4) and the guide repair CLOSED (R24)

- Research: `docs/plans/2026-09-12-green-fresh-install-research.md`, revisions 2–4 (`7b4493e`, `ddafcca`, `a4e8b4d`): ten differences between a fresh Trixie install and what the guide and the code assume; each revision closed the gaps its critic found (eleven, three, five, five); revision 4 closed by ruling (the guide repair's review is the next check). The reviser's fact corrections are in the scratch progress file.
- Guide repair (`briefs/2026-09-13-guide-repair.md`; Opus implementer; report and reviews in scratch): `DEPLOYMENT_GUIDE.md` 1694 → 2886 lines and `docs/preflight-checklist.md` 1692 → 1934 at `7fab0b4`; six documents pointed at the guide; the sweep found 29 candidates and nothing to archive. Task review (Opus): spec fail (narrowly), 1 critical (the checklist's certificate remedy could overwrite the trusted pair), 8 important, 10 minor — all addressed in fix round 1; scoped re-review: 19 addressed, 4 new stale cross-references from the step renumbering, fixed by the orchestrator. Verified fresh before the commit: no stale step references; greps as reported (every hit a "changed from" note); every quoted heading resolves; 40 of 40 template keys documented.
- Ruling 32 (the dormancy pass dispositions; the scratch progress file holds the long form): of twelve standing findings, seven build as one task before any lane merges (`briefs/2026-09-13-dormancy-survivors.md`, worktree `dormancy-fix`); the null-profile major is the ruled profile check (R12, P17–P20); the tier-zero kinds finding goes to the arms brief; the panel's view of `profile.valid` is a ledger row for the preflight panel task. — cost if wrong: a lane merges on a dormancy core with a known defect; the merge gate's integration suite is the net.
- Ruling 33: three code-side facts the guide cannot fix are one small task (`briefs/2026-09-13-green-followups.md`): the boot check accepts the WirePlumber `.conf` path, the two bus variables join the environment template, the requirements file names the apt install. — cost if wrong: one false warning per boot on green, and a template the guide's own audit step flags.
- Appendix C of the roadmap marked closed by R24. Owner note: the guide-ready note is the start-here section at the top of the guide; green may start.

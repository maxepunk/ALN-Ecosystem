...
- **Flow 30 step 1.6.6 (`e2e-video-compound`): root cause is the VLC status signal cache under two instances, not the clip alone.** `vlcMprisService.getStatus()` serves `state` and `length` from a cache fed by MPRIS `PropertiesChanged` signals filtered by `_ownerBusName`; only `Position` is read live. The E2E harness spawns a headless `cvlc --intf dummy` AND the orchestrator, after logging "Existing VLC processes found at init", starts its own instance, so signals from the instance actually playing can be dropped as a sender mismatch until `_refreshOwner()` runs. Verbose run 2026-09-15 (`TEST_LOG_LEVEL=info`): attempt 1 saw "playing" 30s after the play command and never got a progress tick before completion; attempt 2 saw "playing" at once but `length` stayed 0 ("VLC playing but no duration metadata") so the first progress came 31s later. `videoQueueService.monitorVlcPlayback` emits `video:progress` only when `length > 0`, and a boundary-mode cue needs that first tick. Decision (owner, 2026-09-15): cue now plays the existing `test_30sec.mp4` with a 60s wait (no new binary); step is KNOWN FLAKY (1 flaky in 2 runs) until fixed. Fix candidates, to be traced first: (a) harness — stop spawning a second instance (adopt the existing one, or let ProcessMonitor own the only instance with `VLC_HW_ACCEL=""`); (b) service — fall back to a direct `PlaybackStatus`/`Metadata` read when the cache reports stopped/length 0 while a play command is outstanding. `tests/e2e/setup/test-server.js` now honours `TEST_LOG_LEVEL` so the orchestrator's own lines can be captured (`TEST_LOG_LEVEL=info TEST_DEBUG=true`).

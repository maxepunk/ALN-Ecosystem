...
the §15 summary block — the one screen the operator runs on green — which prints `spotifyd: not running` (`:1664`)
...
for the bench. (1) §7 and the §15 binary loop: add chromium, xdotool, wmctrl and mpd — and resolve `CHROMIUM_BIN`
...
`/tmp/aln-mpd.sock`, `musicService.js:47-48`), and in the §15 summary replace the `spotifyd:` line (`:1664`) with an
...
preamble lines and the §15 summary were not. Checked by the guide repair (2026-09-13, `DEPLOYMENT_GUIDE.md` 7fab0b4): 40 keys in `backend/.env.example`, 40 documented, 0 missing (`DEBUG` and `VLC_SELF_SPAWN` were the two never written before). The old note read: the "~28 env keys missing from the guide" reconcile
...
| gate | `docs/preflight-checklist.md` as the acceptance gate (`ROADMAP.md:452-453`) | Run it, after five repairs: chromium (resolved from `backend/.env`, never the shell)/xdotool/wmctrl/mpd in §7 and the §15 loop, a window-control check in place of `xset q` in §8.3, the header path, the `env_production` phantom in §8.4, the `spotifyd` line and the missing MPD line in the §15 summary (§2.12); the proof is the kiosk on the TV | **changed** (checklist repair) |

...
## Full-L3 triage outcome (2026-06-03) — RESOLVED, not deferred
The first full L3 run (136 passed / 49 skipped / 5 failed = 3 distinct tests) was triaged by a 3-lens deep-dive (workflow wyrap75ov): **none of the 3 failures was a comms-fixes regression** (all 3 test files unchanged by the branch). They were FIXED/HARDENED in commit 93a8b4f8 (not deferred):

- **Ducking gate (pre-existing product-config bug, NOT comms-fixes).** `broadcasts.js` gated the video/sound -> ducking/routing wiring on `NODE_ENV !== 'test'` (commit b48ad0f5, 2026-05-23, ancestor of main + HEAD). The E2E orchestrator also runs NODE_ENV=test, so the wiring `07d-04 "Music auto-ducks when video plays"` drives end-to-end was dead — **ducking was unexercised at E2E since 2026-05-23, and 07d-04 fails on main too.** RESOLVED: gate now `ENABLE_AUDIO_WIRES !== 'false'`; jest.config.base.js sets it 'false' (jest layers stay pactl-free, provably identical to the old gate); E2E + prod leave it unset -> wire ON. Confirmed: 07d-04 ducking passes on chromium end-to-end; unit+contract (1739) + 4 audio-wire integration files (44) green.
- **2 video-timing flakes hardened** (admin-state-reactivity "Video State GM1->GM2"; 30-full-game video-compound cue) — load-induced (VLC playing-confirmation latency; vlc-health-flap holding the cue). Plus 07d-04 ducking itself, which the gate fix turned from deterministically-failing into a load-flake. Hardening: test.setTimeout bumps, explicit assertion timeouts, and a pre-trigger `waitForBackendState(serviceHealth.vlc.status==='healthy')` for the cue/video-queue paths (rejected/held when vlc flaps down).

Finding V (GM connectionManager gives up auto-reconnect after maxRetries=5 ~16s) remains the only DEFERRED item from this E2E work — a potential resilience improvement, out of scope for the comms-fixes plan.

## CORRECTION (2026-06-03): admin-state-reactivity root cause was NOT VLC timing
The "Video State GM1→GM2" failure (listed above as a "video-timing flake") was actually a PRE-EXISTING test bug, confirmed deterministic: a raw in-page `page1.evaluate(fetch('/api/state'))` with no timeout that hangs in setup. NOT a comms-fixes regression (the SW bypasses /api/ in both old + new sw.js). Fixed in commit 1bab16e2 by switching to `gm1.getStateFromBackend()` (page.request, SW-bypassing). All 3 originally-failing L3 tests are now fixed/hardened; none was a comms-fixes regression.

...
## Phase 2.x — E2E harness as platform infrastructure (COMMITTED 2026-06-11 — plan: docs/plans/2026-06-11-phase2x-e2e-harness.md)
Sequenced BEFORE the Phase 3 build; co-designed with 3.0/3.1 schemas. Capability manifest + declared per-flow requirements (vocabulary shared with the venue-profile schema / B7-B8 preflight) · suite tiering (Tier L logic/UI = CI floor, Tier H hardware = Pi pre-show gate) · websocket-core event-cache redesign (kills the 31 clearEventCache call sites) · no-fixed-sleeps lint for flows · content-independent fixture/pack injection (first consumer of the pack-loading seam; executes Phase 4's toy-pack + topology acceptance tests). Drift gates (page-object consistency + selector existence) ALREADY LANDED in the unit suite (tests/unit/e2e-harness/).

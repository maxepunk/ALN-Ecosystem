...
- Phase 4 broadcasts: `held:added`/`held:released`/`held:discarded`/`held:recoverable` (unified held item lifecycle), `service:health` (individual service health updates), `service:state` (unified state push with `{domain, state}` envelope — dual-emit alongside existing discrete events)
